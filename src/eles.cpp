...
void eles::shock_capture_llav(int in_disu_upts_from)
{
    if (n_eles != 0) {
#ifdef _GPU
        shock_capture_llav_gpu_kernel_wrapper()
#endif // _GPU

#ifdef _CPU
        shock_capture_llav_cpu(n_eles,n_upts_per_ele,n_fields,order,ele_type,run_input.artif_type,);
#endif
    }
}
