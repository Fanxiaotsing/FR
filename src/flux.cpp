...
//calculate the AV flux in 2D (Peng Yang)

void calc_avf_2d(array<double>& in_u, array<double>& out_f)
{
    double epslion;
    double epslion_zero;
    double delta,xi,h,lambda;
    double alpha, belta;
    double M;
    double c;
    epslion_zero = (alpha * delta * (1 - 1 / M) + belta) * (2.0 - xi) * h * lambda;
